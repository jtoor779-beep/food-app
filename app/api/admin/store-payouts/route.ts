...
async function listRows() {
  const orderColumns = ["created_at", "updated_at"];
...
function buildPatchVariants(base: Record<string, unknown>) {
  ...
  if ("paid_at" in base && "updated_at" in base) {
    const copy = { ...base };
    delete copy.paid_at;
    delete copy.updated_at;
    variants.push(copy);
...
export async function GET(req: Request) {
  try {
    ...
    return NextResponse.json(
      { ok: false, error: error?.message || "Unable to load store payouts." },
      { status: 500 },
    );
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    if (!id || !["requested", "processing", "paid", "failed"].includes(status)) {
    ...
    if (status === "paid") basePatch.paid_at = nowIso;
    ...
    return NextResponse.json(
      { ok: false, error: error?.message || "Unable to update payout status." },
      { status: 500 },
    );
  }
}
